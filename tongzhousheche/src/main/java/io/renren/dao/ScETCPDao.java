...
public interface ScETCPDao extends BaseDao<ScEtcptjdEntity> {
	...
	@Select("<script>"
			 + "select " 
				+ "( SELECT C.color FROM sc_color C WHERE C.VALUE = aaa.VehicleColor ) AS VehicleColor,"
				+ "(SELECT b.contact FROM sc_brand b WHERE b.VALUE= aaa.VehicleBrand LIMIT 1) AS VehicleBrand,"
				...
				+ "(SELECT P.TYPE FROM sc_palte_class_type P WHERE P.VALUE = aaa.PlateClass) AS PlateClass,"
				+ "( SELECT C.color FROM sc_color C WHERE C.VALUE = aaa.PlateColor) AS PlateColor,"

(function() {
	...
	function inputCustomerCSS(style){
        var css = '<' + 'link rel="stylesheet" href="' + baseurl + '../theme/' + style + '"' + '><' + '/>';
        document.writeln(css);
    }
    //加载类库资源文件
    function loadSMLibs() {
		...
		inputScript(baseurl+'jquery-1.8.2.js');
		inputScript(baseurl+'rk_gis.js');
        loadLocalization();
        inputCSS('style.css');
        inputCSS('google.css');
		inputCustomerCSS('rk_map.css');
    }
    //引入汉化资源文件

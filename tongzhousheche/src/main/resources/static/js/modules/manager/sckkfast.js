	     ...
	     ss=sessionStorage.getItem("cph");
		   if (ss!=null&&ss!=""&&ss!="null") {
			   pa=sessionStorage.getItem("pag");
			     ss=ss.toUpperCase();
					ph+=baseURL +"etcp/fastList?cph="+ss;
				...
				pa=sessionStorage.getItem("pag");
			   if (qs!=null&&qs!=""&&qs!="null") {
					ph+=baseURL +"etcp/fastList?qssj="+qs;
					   if (js!=null&&js!=""&&js!="null") {
							ph+="&jssj="+js;
					}	
			}
	...
	methods: {
		...
		reload: function (event) {
			...
			$("#jqGrid").jqGrid('setGridParam',{ 
				url: baseURL + ph,
				postData:{'cph': cph,'numOfPassenger': vm.q.numOfPassenger,'speed': vm.q.speed,'pclass':vm.q.pclass,'qssj':sTime,'jssj':eTime,'color':color,'brand':brand,'qclass':vm.q.qclass,'model':vm.q.model,'infoKind':infoKind,'paltecolor':paltecolor,'safetyBelt':safetyBelt,'calling':calling},
                page:1
            }).trigger("reloadGrid");
		}
	}

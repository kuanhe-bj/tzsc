$(function () {
    ...
    $("#jqGrid").jqGrid({
        url: baseURL + 'generator/sckkjbxx113/list',
        datatype: "json",
        colModel: [
			{ label: 'id', name: 'id', index: 'ID', width: 70, key: true },
			...
			{ label: '拍摄地点', name: 'psdd', index: 'PSDD', width: 320 },
	...
	methods: {
		query: function () {
			...
			if(cph == null || cph == "") {
				...
				$("#jqGrid").jqGrid('setGridParam',{ 
					url: baseURL + 'generator/sckkjbxx113/find',
					postData:{'cph': cph},
					page:1
				}).trigger("reloadGrid");
			}
		...
		saveOrUpdate: function (event) {
			var url = vm.scKkjbxx113.id == null ? "generator/sckkjbxx113/save" : "generator/sckkjbxx113/update";
			vm.scKkjbxx113.rksj = $("#rksj").val();
			vm.scKkjbxx113.pssj = $('#pssj').val();
			//console.log(vm.scKkjbxx113.pssj);
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.scKkjbxx113),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "generator/sckkjbxx113/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "generator/sckkjbxx113/info/"+id, function(r){
                vm.scKkjbxx113 = r.scKkjbxx113;
            });
		},

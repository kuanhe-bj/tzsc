...
$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '首次入城', name: 'isFirstIn', width: 160, 
				formatter:function (cellvalue, options, rowObject) {
				  if(cellvalue == true){
				        return "是";
				    }else{
	...
	methods: {
		query: function () {
			...
			$("#jqGrid").jqGrid('setGridParam',{
				url: baseURL + '/generator/sc_scrc/find',
...
function sx(){
	...
	var message=null;
	$.ajax({
		 url: baseURL + '/generator/sc_scrc/list_dan',
		 dataType:"json",
		 type:"post",
		 async:false,
		 success:function(r){
			 for(var i=0;i<r.list.size;i++){
				 message=r.list[i].plate+"首次进程报警";
				 if(r.list[i]!=null && r.list[i]!=""){					
						info(receiver,message);
					}
			 }
		 }
	});
...
//sc_alarminfo实现报警
function info(receiver,message){
	params={};
	params["receiver"]=receiver;
	params["message"]=message;
	$.ajax({
		url : baseURL + 'generator/zfyc/info',
		dataType:"json",
		data:params,
		type:"post",
		async:false,

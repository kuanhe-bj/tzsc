...
var menuItem = Vue.extend({
    ...
    template:[
        ...
        '</ul>',
        '<a v-if="item.type === 1" :href="\'#\'+item.url" v-if="item.type === 1" :id="item.url" onclick="fresh(id)" >' +
        '<i v-if="item.icon != null" :class="item.icon"></i>' +
        ...
        logout: function () {
            $.ajax({
                ...
                success: function(r){
                    ...
                    //跳转到登录页面
                    location.href = 'http://11.33.29.133:8089/cas/logout?service=http://11.33.29.133:8089/cas/login?service=http://11.33.29.92/vas/index.html';
                }
            });
        },
        donate: function () {
